		...
		for (RuntimeSearchParam nextSpDef : def.getSearchParams()) {
			...
			for (Object nextObject : extractValues(nextPath, theResource)) {
				...
				if (nextObject instanceof IPrimitiveDatatype<?>) {
					...
					ResourceIndexedSearchParamString nextEntity = new ResourceIndexedSearchParamString(resourceName, QueryUtilities.normalizeString(searchTerm), searchTerm);
					nextEntity.setResource(theEntity);
					retVal.add(nextEntity);
				} else {
					if (nextObject instanceof BaseHumanNameDt) {
						...
						for (StringDt nextName : allNames) {
							...
							ResourceIndexedSearchParamString nextEntity = new ResourceIndexedSearchParamString(resourceName, QueryUtilities.normalizeString(nextName.getValueAsString()), nextName.getValueAsString());
							nextEntity.setResource(theEntity);
							retVal.add(nextEntity);
						}
						...
						if (nextContact.getValue().isEmpty() == false) {
							ResourceIndexedSearchParamString nextEntity = new ResourceIndexedSearchParamString(resourceName, QueryUtilities.normalizeString(nextContact.getValue().getValueAsString()), nextContact.getValue().getValueAsString());
							nextEntity.setResource(theEntity);
							retVal.add(nextEntity);
						}
					} else {
			...
			for (Object nextObject : extractValues(nextPath, theResource)) {
				if (nextObject instanceof IdentifierDt) {
					...
					if (!nextCC.getText().isEmpty()) {
						ResourceIndexedSearchParamString nextEntity = new ResourceIndexedSearchParamString(nextSpDef.getName(), QueryUtilities.normalizeString(nextCC.getText().getValue()), nextCC.getText().getValue());
						nextEntity.setResource(theEntity);
						retVal.add(nextEntity);
					}
