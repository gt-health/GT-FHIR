	...
	public List<IResource> transaction(List<IResource> theResources) {
		...
		for (int i = 0; i < theResources.size(); i++) {
			...
			if (res.getId().hasIdPart() && !res.getId().hasResourceType() && !isPlaceholder(res.getId())) {
				res.setId(new IdDt(FhirResourceUtils.toResourceName(res.getClass(), getContext()), res.getId().getIdPart()));
			}
		...
		for (int resourceIdx = 0; resourceIdx < theResources.size(); resourceIdx++) {
			...
			String resourceName = FhirResourceUtils.toResourceName(nextResource, getContext());
			...
			if (StringUtils.isNotBlank(matchUrl)) {
				candidateMatches = DaoUtils.processMatchUrl(matchUrl, nextResource.getClass(), getContext(), getBaseDao().getDao(nextResource.getClass()));
			}
			...
			if (nextResouceOperationIn == BundleEntryTransactionMethodEnum.POST) {
				...
				if (candidateMatches == null || candidateMatches.size() == 0) {
					if (nextId == null || StringUtils.isBlank(nextId.getIdPart())) {
						throw new InvalidRequestException(getBaseDao().getContext().getLocalizer().getMessage(BaseHapiFhirSystemDao.class, "transactionOperationFailedNoId", nextResouceOperationIn.name()));
					}
					...
					if (entity == null) {
						if (nextResouceOperationIn == BundleEntryTransactionMethodEnum.PUT) {
							...
							throw new InvalidRequestException(getBaseDao().getContext().getLocalizer().getMessage(BaseHapiFhirSystemDao.class, "transactionOperationFailedUnknownId", nextResouceOperationIn.name(), nextId));
						} else {
					...
					throw new InvalidRequestException(getBaseDao().getContext().getLocalizer().getMessage(BaseHapiFhirSystemDao.class, "transactionOperationWithMultipleMatchFailure", nextResouceOperationIn.name(), matchUrl, candidateMatches.size()));
				}
			...
			if (entity == null) {
				...
				entity = getBaseDao().toEntity(nextResource);
				if (nextId.isEmpty() == false && "cid:".equals(nextId.getBaseUrl())) {
					...
					if (candidateMatches != null) {
						...
						if (candidateMatches.size() > 1) {
							throw new InvalidRequestException(getBaseDao().getContext().getLocalizer().getMessage(BaseHapiFhirSystemDao.class, "transactionOperationWithMultipleMatchFailure", BundleEntryTransactionMethodEnum.POST.name(), matchUrl, candidateMatches.size()));
						}
					}
				} else {
					DaoUtils.createForcedIdIfNeeded(entity, nextId);
				}
		...
		for (int i = 0; i < persistedResources.size(); i++) {
			...
			String resourceName = FhirResourceUtils.toResourceName(theResources.get(i), getContext());
		...
		for (int i = 0; i < theResources.size(); i++) {
			...
			getBaseDao().updateEntity(resource, table, table.getId() != null, deletedTimestampOrNull);
		}
