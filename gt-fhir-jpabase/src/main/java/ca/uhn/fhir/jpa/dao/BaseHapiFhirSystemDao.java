...
public abstract class BaseHapiFhirSystemDao<T> implements IFhirSystemDao<T> {
	private static final org.slf4j.Logger ourLog = org.slf4j.LoggerFactory.getLogger(BaseHapiFhirSystemDao.class);

	private BaseHapiFhirDao myBaseDao;
	...
	public BaseHapiFhirDao getBaseDao() {
		return myBaseDao;
	}

	public void setBaseDao(BaseHapiFhirDao myBaseDao) {
	...
	protected ResourceTable tryToLoadEntity(IdDt nextId) {
		...
		try {
			Long pid = DaoUtils.translateForcedIdToPid(nextId, myEntityManager);
	...
	public IBundleProvider history(Date theSince) {
		...
		IBundleProvider retVal = myBaseDao.history(null, null, theSince);
	...
	public TagList getAllTags() {
		StopWatch w = new StopWatch();
		TagList retVal = myBaseDao.getTags(null, null);
	...
	public void registerDaoListener(IDaoListener theListener) {
		this.myBaseDao.registerDaoListener(theListener);
	}
